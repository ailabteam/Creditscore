...
# fedavg_mlp_PN.py
# --------------------------------------------------------------
# FedAvg + FedProx  –  MLP classifier với Population Normalization
#   • 10 clients non-IID  (Give-Me-Some-Credit, 20 % local val)
#   • 60 vòng, 3 local-epoch, grad-clip 5
#   • PN tránh NaN (var clamp) – không thêm noise
#   • Log ra console + mlp_PN_run.log
# --------------------------------------------------------------
...
logging.basicConfig(level=logging.INFO,
    ...
    handlers=[logging.StreamHandler(sys.stdout),
              logging.FileHandler("mlp_PN_run.log", mode="w")])
...
X_all = Pipeline([("imp",SimpleImputer(strategy="median")),
...
class MLP_PN(nn.Module):
    def __init__(self,d_in, hidden=(256,128)):
        ...
        cur=d_in
        for h in hidden:
            layers += [nn.Linear(cur,h), PN1d(h), nn.ReLU()]
            cur=h
        layers.append(nn.Linear(cur,1))           # logit
        self.net = nn.Sequential(*layers)
    def forward(self,x): return self.net(x)

global_model = MLP_PN(len(FEATS)).to(DEVICE)

getP = lambda m: [p.detach().cpu().numpy() for p in m.state_dict().values()]
def setP(m, params):
    m.load_state_dict({k: torch.tensor(v) for k,v in zip(m.state_dict(), params)}, strict=True)

bce = nn.BCEWithLogitsLoss()
momentum = None
log.info(f"Device {DEVICE} | MLP + PN | {ROUNDS} rounds × {LOCAL_EPOCH} epoch\n")
...
for rd in range(1, ROUNDS+1):
    ...
    for (Xt,yt), _ in clients:
        local = MLP_PN(len(FEATS)).to(DEVICE)
        ...
        sampler = WeightedRandomSampler(np.where(yt==1,0.7,0.3), len(yt), replacement=True)
        ...
        new_weights.append(getP(local)); sizes.append(len(yt))

    # -------- aggregation (FedAvg + momentum) --------
    tot = sum(sizes)
    avg = [sum(w*n for w,n in zip(layer, sizes))/tot for layer in zip(*new_weights)]
    if momentum is None:
